...
import { ClockFading, MapPin, CheckCircle2, ScanLine } from "lucide-react";
...
function itemUnitPriceCents(it) {
  return (
    it?.menuItem?.priceCents ??
    it?.menuItem?.price_cents ??
    0
  );
}
...
function normalizeOrdersResponse(data) {
  if (!data) return { incoming: [], pending: [], ready: [] };
  ...
  // ✅ keep ready orders visible under Pending Orders
  const mergedPending = [...pending, ...ready];

  return { incoming, pending: mergedPending, ready };
}
...
function splitByStatus(list) {
  ...
  for (const o of list) {
    ...
    if (os === "awaiting" || os === "pending" || os === "new") {
      incoming.push(o);
    } else if (os === "preparing" || os === "ready") {
      pending.push(o);
      if (os === "ready") ready.push(o);
    }
  }

  return { incoming, pending, ready };
}
...
function computeDefaultEstimateMins(order) {
  ...
  const sum = items.reduce(
    (acc, it) => acc + itemPrepMins(it) * itemQty(it),
    0
  );
...
function OrderRow({ order, rightBadge, onClick }) {
  ...
  const notes =
    first && itemNotes(first) ? itemNotes(first) : "No special instructions";

  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full text-left bg-white rounded-lg shadow-[0_2px_8px_rgba(0,0,0,0.12)] hover:shadow-[0_4px_14px_rgba(0,0,0,0.16)] transition px-6 py-5 flex items-center justify-between"
    >
      ...
      <div className="flex-shrink-0 pr-4 self-stretch flex items-center justify-center">
        {rightBadge}
      </div>
    </button>
  );
...
export default function HawkerOrdersPage() {
  ...
  async function fetchOrders() {
    try {
      ...
      const grouped = normalizeOrdersResponse(res.data);
  ...
  function openChecklist(order) {
    ...
    const items = safeItems(order);
    setCheckedIds(
      new Set(items.filter((it) => it.isPrepared).map((it) => String(it.id)))
    );
  }
  ...
  async function togglePrepared(it) {
    ...
    try {
      await api.patch(
        `/hawker/orders/${activeOrder.id}/items/${id}/prepared`,
  ...
  async function handleMarkReady() {
    ...
    try {
      setSaving(true);
      const res = await api.patch(`/hawker/orders/${activeOrder.id}/ready`, {});
      const pickupToken = res?.data?.pickupToken;
      closeModal();
      await fetchOrders();
      if (pickupToken) {
        alert(`Pickup token generated: ${pickupToken}`);
      }
    } catch (e) {
      setSaving(false);
  ...
  const totalCents =
    activeOrder?.totalCents ?? Math.max(0, subtotalCents + serviceFeeCents - voucherCents);

return (
  <div className="w-full">
    <div className="mx-auto max-w-7xl px-6 py-5">
      <div className="flex items-center justify-center mb-4">
        <div className="inline-flex rounded-md border border-gray-200 bg-white p-[2px]">
          <button className="px-6 py-2.5 rounded-[6px] bg-[#21421B] text-white text-xs font-semibold">
            Current
          </button>
          <button className="px-6 py-2.5 rounded-[6px] text-gray-600 text-xs font-semibold">
            History
          </button>
        </div>
      </div>
      ...
      <div className="space-y-5 mb-12">
          ...
          pending.map((o) => {
            ...
            return (
              <OrderRow
                ...
                order={o}
                rightBadge={
                  <div className="relative min-w-[120px] h-[72px]">
                    ...
                    <div className="absolute left-0 right-0 bottom-0 text-xs text-gray-500 text-center">
                      {isReadyRow ? "Scan QR" : fmtCountdown(remain)}
                    </div>
                  </div>
                }
        ...
        {activeOrder && activeMode === "accept" ? (
          <ModalShell
            title="Order Details"
            subtitle={`Ordered on ${fmtDateTime(
              activeOrder.createdAt || activeOrder.created_at
            )}`}
            onClose={closeModal}
          >
              ...
              <div className="mt-6 border-t pt-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-sm text-gray-700">
                  ...
                  <span className="font-semibold text-gray-900">
                    -{fmtMoneyFromCents(voucherCents)}
                  </span>
                </div>
              ...
              <div className="mt-3 border-t pt-3">
                ...
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div className="text-4xl font-extrabold">
                    {estimateMins}{" "}
                    <span className="text-lg font-semibold text-gray-500">mins</span>
                  </div>
                ...
                <div className="mt-6 flex items-center justify-center gap-4">
                  <button
                    type="button"
                    className="px-10 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-semibold"
                    onClick={closeModal}
                    disabled={saving}
                  >
                    Reject
                  ...
                  <button
                    type="button"
                    className="px-10 py-2.5 rounded-lg bg-[#21421B] text-white font-semibold hover:bg-[#1A3517]"
