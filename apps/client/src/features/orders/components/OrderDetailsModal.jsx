...
export default function OrderDetailsModal({ order, onClose }) {
  ...
  const completedAtRaw = order?.completedAt || order?.completed_at || order?.updatedAt;
  ...
  if (estimatedReadyTime) {
    ...
    if (!isNaN(d)) {
      estimatedPickupText = `Around ${d.toLocaleTimeString("en-SG", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false,
      })}`;
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="space-y-2">
              ...
              {(status?.toLowerCase() === "completed" ||
                ...
                <p className="text-sm text-gray-500">Collected on: {collectedAtText}</p>
              )}
            </div>
            ...
            {/* Pick Up QR (always reserve space) */}
